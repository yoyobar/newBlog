...
<Preview>
    <p className="inline-block bg-green-500 w-[50px] h-[50px]">생활</p>
</Preview>

예시로 `<p>` 라는 **인라인 요소**에 inline-block을 할당해뒀다. 
원래 `<p>` 라는 **인라인 요소**에는 길이 관련된 속성은 붙을 수 없지만
실제로는 적용된걸 확인할 수 있다.
...
<Preview>
<div className="bg-slate-400 p-2 rounded-md hover:bg-slate-700 hover:text-white duration-500 delay-100 transition linear">
Test
</div>
</Preview>
...
<Preview>
<div className="w-[50px] h-[50px] bg-slate-600 animate-spin delay-200">살려줘</div>
</Preview>
